...
async function getColleges(formData) {
    ...
    // Clean the caste_condition properly
    let cleanCasteCondition = caste_condition.trim();
    
    // Replace multiple "TRUE AND" with just "1=1 AND"
    cleanCasteCondition = cleanCasteCondition.replace(/(TRUE\s+AND\s*)+/g, '1=1 AND ');
    
    // Remove trailing AND temporarily
    cleanCasteCondition = cleanCasteCondition.replace(/\s+AND\s*$/, '');
    
    // If the condition is empty or just "TRUE", use "1=1"
    if (!cleanCasteCondition.trim() || cleanCasteCondition.trim() === 'TRUE') {
        cleanCasteCondition = '1=1';
    }
    
    // CRITICAL: Always ensure the condition ends with AND for proper SQL syntax
    cleanCasteCondition = cleanCasteCondition.trim() + ' AND';
    
    // Clean the caste_column - remove trailing comma since database function adds it
    let cleanCasteColumn = caste_column.trim();
    cleanCasteColumn = cleanCasteColumn.replace(/,\s*$/, '');
    
    // console.log("Generated SQL:", cleanCasteColumn);
    // console.log("caste condition UPDATED:", cleanCasteCondition);
    // console.log("Length of condition:", cleanCasteCondition.length);

    try {
        // First query
        const { data: mhtData, error: mhtError } = await supabase.rpc('engimate_preference_list', {
            ...
            caste_column: cleanCasteColumn,
            caste_condition: cleanCasteCondition
        });
        
        if (mhtError) throw mhtError;

        // Second query
        const { data: allIndiaData, error: allIndiaError } = await supabase.rpc('all_india_engimate', {
            homeuniversity: formData.homeuniversity,
...
router.post('/College_list', async (req, res) => {
    ...
    try {
        ...
        colleges_1.sort((a, b) => a.points - b.points);
        ...
        colleges_2.sort((a, b) => b.points - a.points);


        let college_counts = 150;
